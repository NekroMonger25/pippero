...
async function getVideoPage(url: URL): Promise<Response> {
    try {
...
export async function evalResolver(link: URL): Promise<string | null> {
    try {
        ...
        let script: string | null = null;$('script').each((_: number, element: any) => {
